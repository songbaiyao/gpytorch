...
class TestCachedCGLazyTensor(LazyTensorTestCase, unittest.TestCase):
    ...
    def create_lazy_tensor(self):
        ...
        eager_rhs = torch.randn(5, 10)
        return CachedCGLazyTensor(NonLazyTensor(mat), eager_rhs)
    ...
    def test_inv_matmul_vec(self):
        ...
        test_vector = lazy_tensor.eager_rhs[..., 0].clone().detach().requires_grad_(True)
        test_vector_copy = lazy_tensor_copy.eager_rhs[..., 0].clone().detach().requires_grad_(True)
    ...
    def test_inv_matmul_vector_with_left(self):
        ...
        test_vector = lazy_tensor.eager_rhs[..., 0].clone().detach().requires_grad_(True)
        test_vector_copy = lazy_tensor_copy.eager_rhs[..., 0].clone().detach().requires_grad_(True)
        test_left = lazy_tensor.eager_rhs[..., 1:].t().clone().detach().requires_grad_(True)
        test_left_copy = lazy_tensor_copy.eager_rhs[..., 1:].t().clone().detach().requires_grad_(True)
        # Make sure that we get no warning about CG
        with gpytorch.settings.max_cg_iterations(200), warnings.catch_warnings(record=True) as w:
    ...
    def test_inv_matmul_matrix(self):
        ...
        test_vector = lazy_tensor.eager_rhs[..., :2].clone().detach().requires_grad_(True)
        test_vector_copy = lazy_tensor_copy.eager_rhs[..., :2].clone().detach().requires_grad_(True)
        # Make sure that we get no warning about CG
    ...
    def test_inv_matmul_matrix_with_left(self):
        ...
        test_vector = lazy_tensor.eager_rhs[..., :2].clone().detach().requires_grad_(True)
        test_vector_copy = lazy_tensor_copy.eager_rhs[..., :2].clone().detach().requires_grad_(True)
        test_left = lazy_tensor.eager_rhs[..., 3:].transpose(-1, -2).clone().detach().requires_grad_(True)
        test_left_copy = lazy_tensor_copy.eager_rhs[..., 3:].transpose(-1, -2).clone().detach().requires_grad_(True)
    ...
    def test_root_inv_decomposition(self):
        ...
        test_mat = lazy_tensor.eager_rhs[..., :5].clone().detach()
